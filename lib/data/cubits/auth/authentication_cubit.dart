  ...
  class AuthenticationCubit extends Cubit<AuthenticationState> {
    ...
    Future<void> _setupAuthentication() async {
      print("========> _setupAuthentication START <========");
      ...
      print("==> Active authentication type: ${type!.name}");

      print("==> Setting active system in mMultiAuthentication...");
      mMultiAuthentication.setActive(type!.name);
      print("==> setActive() called with: ${type!.name}");

      print("==> Preparing payload for MultiLoginPayload...");
      ...
      print("==> MultiLoginPayload set successfully.");

      print("========> _setupAuthentication END <========");
    }
    ...
    Future<void> _handleStandardAuthentication() async {
      print("========> _handleStandardAuthentication START <========");

      final credential = await mMultiAuthentication.login();
      print("==> Credential returned from mMultiAuthentication.login(): $credential");

      if (credential == null) {
        print("==> No credential returned. Login failed or cancelled.");
        return;
      }

      if (_isEmailLoginWithUnverifiedEmail(credential)) {
        print("==> Detected unverified email login.");
        _handleUnverifiedEmail();
      } else {
        print("==> Login successful. Emitting AuthenticationSuccess.");
        emit(AuthenticationSuccess(type!, credential, payload!, null));
      }

      print("========> _handleStandardAuthentication END <========");
    }
